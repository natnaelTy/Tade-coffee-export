import { Link } from "react-router-dom"
import './style.css';
import { FaInstagram } from "react-icons/fa6";
import { FaFacebook } from "react-icons/fa6";
import { FaXTwitter } from "react-icons/fa6";
import { FaPhoneAlt } from "react-icons/fa";
import { MdOutlineMail } from "react-icons/md";
import { IoLocationSharp } from "react-icons/io5";

function Footer(){

    return(
        <div className="coffee-map relative">
            <div className="pt-10 px-4 md:px-10 lg:px-16 w-full overflow-hidden">
            <div className="mx-auto flex flex-col justify-between md:flex-row p-2 gap-10 text-white">
                <div className="flex items-start flex-col justify-start">
                     <div className="relative w-full h-[100px] flex items-center justfiy-around ">
                       <img src="./images/coffeelogo.png" alt=""  className="w-[70px] lg:w-[90px] object-cover  mb-6"/>
                       <h1 className=" text-sm md:text-lg uppercase font-medium">Tade coffee export</h1>
                     </div>
                     <p>Speciality coffee from the land <br /> of origin.</p>
                </div>
             
                <div>
                    <p className="text-2xl lg:text-3xl font-semibold mb-8">Usefull Links</p>
                    <ul className="flex flex-col gap-4 items-start">
                        <button><Link to={'/'}>Home</Link></button>
                        <button><Link to={'/aboutUs'}>About</Link></button>
                        <a href="">What we offer</a>
                        <button><Link to={'/ordering-info'}>Info (FAQ)</Link></button>
                        <button><Link to={'/contact'}>Contact</Link></button>
                    </ul>
                </div>
                <div>
                    <h1 className="text-2xl lg:text-3xl font-semibold mb-8">Socials</h1>
                    <ul className="flex flex-col gap-8 text-xl hover:border-b-0">
                        <li className="hover:text-blue-700"><a href=""><FaFacebook/></a></li>
                        <li className="hover:text-pink-700 "><a href=""><FaInstagram/></a></li>
                        <li className="hover:text-black"><a href=""><FaXTwitter/></a></li>
                    </ul>
                </div>
                <div className="flex flex-col gap-8 mb-6">
                    <h1 className="text-2xl lg:text-3xl font-semibold">Contact us</h1>
                    <div className="flex gap-4 items-center  justify-start">
                        <FaPhoneAlt className="text-lg"/>
                        <p>+251 975329588</p>
                    </div>
                    <div className="flex gap-4 items-center justify-start">
                        <MdOutlineMail className="text-lg"/>
                        <p>tadeinfo@gmail.com</p>
                    </div>
                    <div className="flex gap-4 items-center justify-start">
                         <IoLocationSharp className="text-lg"/>
                        <p>Semit, Addis Ababa</p>
                    </div>
                  </div>
                  </div>
                        <div className="copyright text-white p-6 py-6 text-sm text-center">Copyright © 2024. All right reserved. Tade Coffee Export</div>
                </div>
              </div>
    )
}

export default Footer;